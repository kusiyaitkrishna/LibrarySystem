...
def borrow_list(request):
    ...
    records = BorrowRecord.objects.all().select_related('book', 'member', 'member__user')
...
def borrow_book(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'{book.title} issued to {borrow_record.member.member_id}')
...
def borrow_detail(request, pk):
    ...
    record = get_object_or_404(
        BorrowRecord.objects.select_related('book', 'member', 'member__user'),
        pk=pk
    )
